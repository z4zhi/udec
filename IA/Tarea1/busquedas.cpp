 ...
 */
void print(const vector<string> &camino, int costo)
{
    const static int optimo = 18; // A -> B -> D -> H
    vector<string> vs;            // los nodos visitados
    vector<int> vi;               // cantidad de veces visitados/expandidos
    ...
    for (int i = 0; i < camino.size(); i++)
    {
        ...
        int f = 0; // comprobar si el nodo ha sido agregado al vectos vs, sino agregarlo
        ...
        if (!f)
            vs.push_back(camino[i]), vi.push_back(1);
    }
    cout << endl
         << "Costo: " << costo;
...
void cu(nodo *actual)
{
    ...
    print(cgoal.camino, cgoal.costo);
}
